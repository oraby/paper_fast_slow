...
| **C** | Behaviour tests | **Code done, C1–C7; one open decision.** Every extracted panel reproduces its committed figure and is tested; `figcode/`, `opto/` and `tracking/` went 0 → 112, 51 and 48 tests. **Open:** the S3 tracking panels disagree with their legend and Methods text (`manuscript-issues.md` #9–#13), and for S3M the conclusion reverses depending on which is intended (0/4 vs 4/4 mice). Also still inline: `Tracking.ipynb`'s preprocessing, which pandas 3 would break silently |
