...
def mle_fit_psycho(data, P_model='weibull', parstart=None, parmin=None, parmax=None, nfits=5, ncpus=None):
    """
    ...
    parstarts = parmin + np.random.rand(nfits-1, parmin.size) * (parmax-parmin)
    parstarts = np.vstack((parstart, parstarts))
    mapFn = functools.partial(_wrapRun, data=data[:, ii], P_model=P_model, parmin=parmin, parmax=parmax)
    if ncpus is None or ncpus <= 1:
        pars_likelihoods = map(mapFn, parstarts)
    else:
        pool = multiprocessing.Pool(ncpus)
        pars_likelihoods = pool.map(mapFn, parstarts)
        pool.close()
        pool.join()
    pars, likelihoods = zip(*pars_likelihoods)
    pars = np.asarray(pars)
    iBestFit = np.argmax(likelihoods)
    return pars[iBestFit, :], likelihoods[iBestFit]

def _wrapRun(parstart, data, P_model, parmin, parmax):
    """
    Wrapper for scipy.optimize.minimize to be used with multiprocessing.
    """
    f = functools.partial(neg_likelihood, data=data, P_model=P_model,
                          parmin=parmin, parmax=parmax)
    pars = scipy.optimize.fmin(f, parstart, disp=False)
    likelihood = -f(pars)
    return pars, likelihood
