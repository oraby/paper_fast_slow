from .plotdefinitions import (legendLabelAndLineStyle, AllClrFn, Difficulty_clr,
                              Difficulty_Direction, DirectionClrFn,
                              ChoiceOutcomeClrFn)
from ._imaging import (avgEpochs, plotAreas, plotHeatMap, splitLevel,
                       SplitLevelBy, savePrefixTmplt)
...
def buildCacheKey(cur_fig_save_prefix, save_subdir, split_level,
    ...
    if align_feedback_time:
        align_part = (f"_align_{align_feedback_time}s"
                      f"{'_norm_limited_end' if limit_end_epoch else ''}")
    else:
        align_part = ""
...
class PlotCombination:
    ...
    avg_kargs: dict
    plt_clr: Any
    more_cond: Union[Condition, List[Condition]] = None
    ...
    def getCombination(conditions=[], **filters):
        ...
        for comb in Combinations.__dict__.values():
            ...
            if comb.avg_kargs == filters and sorted(comb_cond_li) == conditions:
                return comb
        raise ValueError(f"Combination not found for conditions: {conditions} "
...
def plotNormalized(df_concat_norm, df_concat_without_feedback, combinations,
    ...
    # df_concat_norm = df_concat_norm[df_concat_norm.ShortName ==
    #                                 "GP4_23_S1_L50_D250_mm2"]
...
def _getOnlyTracesIds(onlyTraceIdsPlotProcessFn, only_traces_ids, df,
    ...
    if isinstance(split_by, pipeline.By):
        ...
        for grp_vals, grp_df in groupby:
            if not isinstance(grp_vals, list) and not isinstance(grp_vals,
                                                                 tuple):
        ...
        # grouper = [({key:grp_val for key, grp_val in zip(keys, grp_vals)},
        #             grp_df)
    ...
    for group_info, grp_df in grouper:
        for row_idx, row in grp_df.iterrows():
            ...
            for set_name, traces_dict in pipeline.getRowTracesSets(row).items():
                traces_ids_set = onlyTraceIdsPlotProcessFn(set_name,
                                                          traces_dict.keys(),
...
def avgEpochsStub(comb, df, cached_df_avgs, normalization,
    ...
    if save_figs_outliers:
        savePrefix = partial(savePrefixTmplt,
...
def plotTracesStub(comb, df, getAvgFn, normalization, cur_fig_save_prefix,
                   split_level, save_figs, x_label, y_label, only_traces_ids=[],
    ...
    assert not (renameSaveFn is not None and appendSaveFn is not None), (
      "can only specify one of 'renameSaveFn=' or 'preappendSaveFn=' at a time")
    normalization_str = normalization.asStr()
    ...
    savePrefix = partial(savePrefixTmplt, fig_short_name="",#"avg",
    ...
    plotAreas(df_side=None, df_unified=df_avg_comb.copy(), use_unified=True,
              split_level=split_level, x_label=x_label, y_label=y_label,
              only_traces_ids=cur_only_traces_ids, loopGroupTracesFn=None,
...
def heatmapStub(comb, df, getAvgFn, normalization, cur_fig_save_prefix,
                ...
                only_common_sort_not_common_rng=sig(plotHeatMap).parameters[
                                     "only_common_sort_not_common_rng"].default,
                save_ext=None, fig_postfix="", **plot_kargs):
    df_avg_comb, *_ = getAvgFn(comb, df=df)
    # display_epochs, extra_prefix = ["Wait Trial Start",
    #                                 "Sampling"], "pre_n_sampling"
    # display_epochs, extra_prefix = ["Sampling"], "only_sampling"
    display_epochs= []
    # display_epochs, extra_prefix = ["Wait Trial Start", "Sampling",
    #                             "Movement to Lateral Port"], "until_movement"
    ...
    if normalization_str == "without":
        normalization_str = None
    savePrefix = partial(savePrefixTmplt,
                         fig_short_name=f"hm_{save_extra_prefix}{comb.name}_",
                         #plot_id=plot_id,
                         fig_save_prefix=cur_fig_save_prefix, #df=plot_df,
                         is_unified=False,
                         is_widefield=False,
    ...
    if (#comb != Combinations.All and # For testing
        ...
        def heatMapSortSrcDf(df, by_split_level, split_kws, match_cols):
            ...
            return (all_df_avg_comb, match_cols, by_split_level,
                    is_df_one_of_them)
...
def widefieldVidStub(comb, df, getAvgFn, normalization, cur_fig_save_prefix,
    ...
    if normalization_str == "without":
        normalization_str = None
    savePrefix = partial(savePrefixTmplt, fig_short_name="",
                         fig_save_prefix=cur_fig_save_prefix, #df=plot_df,
    ...
    def savePrefixGlue(plot_id, session_name, df=None):
            return savePrefix(plot_id=plot_id, parent_dir=session_name, df=df)
    # print("only_traces_ids:", only_traces_ids)
    df_avg_comb, _, plot_title_postfix = getAvgFn(comb, df=df)
    cur_only_traces_ids = _getOnlyTracesIds(onlyTraceIdsPlotProcessFn,
                                            only_traces_ids, df_avg_comb,
    ...
    if not activity_plot_vid:
        vid_gen = wf_signal.GenVid(assign_mov=False,
                                   save_prefix=savePrefixGlue,
                                   overlay_text=False)
    else:
        vid_gen = wf_signal.SaveActivityPloltVid(is_avg_trc=True, dpi=300,
                                                 only_traces_ids=
                                                            cur_only_traces_ids,
                                                 save_tiff=False,
                                                 plot_y_lims=
                                                           activity_plot_y_lims,
                                                 plot_max_x=
                                                        activity_plot_vid_max_x,
                                                 save_prefix=savePrefixGlue,
                                                 plot_fig_size=None,
                                                 **plot_kargs)
...
def _prepareDF(df, comb_name):
    ...
    df["SaveName"] = df["TrialNumber"]\
                                    .str.replace("epoch_Wait Trial Start", "")\
