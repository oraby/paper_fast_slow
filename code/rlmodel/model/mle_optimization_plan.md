...
| D20 | **O6** — Factorized time-varying mu for Decay-Q variants. The `(n_candidates × n_sessions × trials × n_t)` mu tensor (~7 GB on a typical population fit) is gone — Decay-Q latents now carry only `(n_trials,)` per-trial scalars and `(n_candidates, n_t)` decay shapes; the solver computes `mu_t` per step on-device via column gather + per-trial scale. Memory reduction: ~7 GB → ~25 MB on a typical Decay-Q population fit. | ✅ done | [mle_batch.py](mle_batch.py) `TimeVaryingMuFactors`, `_SolverMuFactorsXP`; [mle.py](mle.py) `_compute_latent_population_equal_sessions` Decay-Q branch; `objective_from_population` factored-mu pass-through |
