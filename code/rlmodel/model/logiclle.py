...
def chi2Loss(real_rt, sim_rt, t_dur):
    ...
    # Get quantile to include only one item in the first bin
    first_item_quantile = 1 / (real_rt.shape[0] + 1)
    ratcliff_quantiles = [first_item_quantile, .1, .3, .5, .7, .9]
    ...
    # real_rt = real_rt.values
    # sim_rt = sim_rt.values
    ...
    # print("Real bins:", bins)
    # print("Real freq:", freq_real[0], " - Sim freq:", freq_sim[0])
    # print("Real count:", (real_rt <= bins[1]).sum(), " - Sim count:", (sim_rt <= bins[1]).sum())
...
def calcLoss8(df, dt, t_dur, is_loss_no_dir):
    df = df[df.valid]
    loss = 0
    for choice_correct in (1, 0):
        ...
        if is_loss_no_dir:
            ...
            for choice_left in (1, 0):
                ...
                loss += chi2Loss(real_choice_rt, sim_choice_rt, t_dur)
        # real_choice_rt = df[df["ChoiceCorrect"] == choice_correct].calcStimulusTime
        # sim_choice_rt = df[df["SimChoiceCorrect"] == choice_correct].SimRT
        # loss += chi2Loss(real_choice_rt, sim_choice_rt, t_dur)
    return loss
